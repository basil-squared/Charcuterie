...
extern MY_HIGHP_OR_MEDIUMP vec2 testshader;
...
void effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    // Create waving pattern
    float wave = sin(uv.x * waveDensity * 6.2831 + time * waveSpeed) *
    cos(uv.y * waveDensity * 3.1415 + time * waveSpeed) *
    waveIntensity;

    // Create radial gradient
    vec2 center = uv - 0.5;
    float radius = length(center) * 2.0;
    float gradient = smoothstep(0.5 - edgeFade, 0.5 + edgeFade, radius);

    // Combine effects
    float wavePattern = wave * (1.0 - gradient);
    vec3 color = vec3(1.0, 0.0, 0.0); // Red color

    // Alpha calculation
    float alpha = smoothstep(0.3, 0.5, wavePattern) * 0.7;
