...
extern MY_HIGHP_OR_MEDIUMP vec2 melty;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    uv.x *=1.0 + sin(melty.y + time * 0.25) / 5.5 * sin(uv.y * 15.0);
    uv.y +=0.2 * cos(uv.x * 10.0) / 5.0 * sin (melty.y + time * 0.001);


    // vec3 col = pow(cos(15.0 * tex.rgb), vec3(1.0));

        // Output to screen


    vec4 final_color = vec4(col, clamp(melty.x, 1.0, tex.a));
